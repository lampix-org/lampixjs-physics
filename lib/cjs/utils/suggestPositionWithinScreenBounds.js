...
const randomAlphaMinMaxDeg_1 = require("./randomAlphaMinMaxDeg");
...
function suggestPositionWithinScreenBounds(ms, theOptions) {
    ...
    let alpha = randomAlphaMinMaxDeg_1.randomAlphaMinMaxDeg((theOptions.index * 90) + 90, (theOptions.index * 90) + 120);
    ...
    do {
        if (destx < (theOptions.w / 2) || desty < (theOptions.h / 2) ||
            ...
            if (destx < (theOptions.w / 2)) {
                alpha = randomAlphaMinMaxDeg_1.randomAlphaMinMaxDeg(theOptions.index, 30 + theOptions.index * 45);
            }
            if (desty < (theOptions.h / 2)) {
                alpha = randomAlphaMinMaxDeg_1.randomAlphaMinMaxDeg(theOptions.index * 45, 30 + theOptions.index * 90);
            }
            if (destx > ms.setup.width - (theOptions.w / 2)) {
                alpha = randomAlphaMinMaxDeg_1.randomAlphaMinMaxDeg(theOptions.index * 180, 30 + theOptions.index * 245);
            }
            if (desty > ms.setup.height - (theOptions.h / 2)) {
                alpha = randomAlphaMinMaxDeg_1.randomAlphaMinMaxDeg(theOptions.index * 245, 30 + theOptions.index * 300);
            }
...
exports.suggestPositionWithinScreenBounds = suggestPositionWithinScreenBounds;
