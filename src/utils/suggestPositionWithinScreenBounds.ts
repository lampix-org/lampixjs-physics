...
export function suggestPositionWithinScreenBounds(ms: any, theOptions: ObjectIdentifiers) {
  ...
  let destx = theOptions.cx + Math.cos((alpha + 60) / 180 * Math.PI) * linelength;
  let desty = theOptions.cy + (Math.sin((alpha + 60)  / 180 * Math.PI) * linelength * -1);
  ...
  do {
    ...
    desty > ms.setup.height - (theOptions.h / 2)) {
      ...
      destx = theOptions.cx + Math.cos(alpha / 180 * Math.PI) * linelength;
      desty = theOptions.cy + (Math.sin(alpha / 180 * Math.PI) * linelength * -1);
    }
