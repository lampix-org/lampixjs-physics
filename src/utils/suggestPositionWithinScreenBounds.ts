...
export function suggestPositionWithinScreenBounds(theOptions: ObjectIdentifiers) {
  ...
  do {
    ...
    destx > MatterSetup.setup.width - (theOptions.w / 2) || 
    desty > MatterSetup.setup.height - (theOptions.h / 2)) {
      ...
      if (destx > MatterSetup.setup.width - (theOptions.w / 2)) {
        alpha = randomAlphaMinMaxDeg(theOptions.index * 180, 30 + theOptions.index * 245);
      }
      if (desty > MatterSetup.setup.height - (theOptions.h / 2)) {
